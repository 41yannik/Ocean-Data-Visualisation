...
export function createSelectionSummary(container, ctx) {
  ...
  const renderEvent = (event) => {
    const deaths = event.deaths == null ? 'not reported' : fmtInt(event.deaths);
    const coverage = event.deaths == null
      ? 'Affected toll available; deaths missing'
      : 'Affected and death tolls available';
    container.innerHTML = `<p class="kicker">Selected record</p><h3>${event.name ?? 'Unnamed storm'} · ${event.country}</h3>
      ...
      <div><dt>Deaths</dt><dd>${deaths}</dd></div><div><dt>Data coverage</dt><dd>${coverage}</dd></div></dl>
