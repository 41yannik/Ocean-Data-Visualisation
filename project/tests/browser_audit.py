...
HARNESS_MOUNTS = (
    "tooltip", "detail", "toggle", "filters", "scatter.axes",
    "scatter.points", "scatter.trend", "scatter", "map.basemap", "map.tracks",
    "map.centroids", "map", "data", "sst", "story",
    "residualLab", "tollMap",
...
def audit_page(browser, viewport):
    ...
    # Harold hook: one map frame; the two impact bubbles pop AFTER the camera
    # flight (regression for the self-popping impact layer) and carry labels.
    ...
    assert page.locator("#step-2 .impact-label").evaluate_all(
        "els => els.every(el => Number(el.getAttribute('opacity')) === 1)")
    hook_text = page.locator("#step-2 .impact-label").all_text_contents()
    assert any("of population" in (t or "") for t in hook_text)
    ...
    # Evidence panel keeps its interactive controls (3 highlight buttons + dropdown).
    assert page.locator("#step-3 .cc-btn").count() == 3
    harold_btn = page.locator('#step-3 .cc-btn[data-key="harold"]')
    harold_btn.click()
    assert harold_btn.get_attribute("aria-pressed") == "true"
    harold_btn.click()
    assert harold_btn.get_attribute("aria-pressed") == "false"
    ...
    if viewport["name"] == "desktop":
        ...
        # The visual now has two equal macro-columns: both Top-5 lists together
        # and the paired thermometer. Comparing one half-list to the complete
        # thermometer encoded the superseded three-column layout.
        assert abs(layout["thermo"]["width"] - layout["lists"]["width"]) <= 3
    ...
    for key in ("intro", "synthesis", "board"):
    ...
    assert page.evaluate("window.store.get().activeYear") == 2007
    ...
    assert page.locator("#detail.open").count() == 1
    assert page.locator("#detail .dp-stats > div").count() == 3
    ...
    assert "country-year record" in page.locator("#detail .dp-reading").inner_text().lower()
    ...
    assert page.locator("#methods .methods-source").count() == 5
