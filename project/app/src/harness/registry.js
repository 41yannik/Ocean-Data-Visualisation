...
tracks:      ${Object.keys(tracks).length}   (erwartet 53)
scatterable: ${scatterable}   (erwartet 71)
...
Harold-Zeilen: ${index.bySid.get('2020092S09155')?.length} (erwartet 2: VUT + FJI)</pre>`;
  ...
  chartControls: {
    title: 'Chart-Controls (Evidence-Panel): 3 Highlight-Buttons + Land-Dropdown',
    mount(c, ctx) {
  ...
  'story.text': {
    ...
    mount(container, ctx) {
      ...
      let selfTest;
      try {
        resolveRefs('{{event:XXX-9999.affected:int}}', ctx);
        selfTest = 'FAIL: unbekannte Referenz hat NICHT geworfen!';
      } catch (err) {
        selfTest = `PASS: wirft wie erwartet: „${err.message}"`;
      }
      container.innerHTML = `
        <pre class="harness-summary">Steps: ${steps.length} (erwartet 11) · Selbsttest ungültige Referenz: ${selfTest}</pre>
        ${steps.map((s, i) => {
          const patch = s.apply();
