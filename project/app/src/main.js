...
// IntersectionObserver mountet eine Sektion kurz bevor ihre Grafik sichtbar wird.
    ...
    <div class="evidence-lab">
      ...
      <div class="evidence-panels">
        ...
        <section id="evidence-countries" class="evidence-panel" data-panel="countries" role="tabpanel" aria-labelledby="question-countries" hidden>
          <header><h3 id="question-countries">Which countries appear repeatedly in the impact records?</h3><p>Each dot is one country-year with a reported toll; its fill shows how large that toll was.</p></header>
          <div class="evidence-empty" hidden><p role="status">No country-year records match these filters.</p><button type="button" data-clear-filters>Clear filters</button></div>
...
async function runApp() {
  ...
  createFullscreenToggle(document.body);
  try {
    ...
    function mountSection(sectionEl, sec) {
      ...
      // Innerhalb der Sektion propagieren (z. B. Highlight-Buttons des Evidence-Panels).
      // Das Story-Gate bleibt zu; Explore-Interaktion ist durch exploreUnlocked:false gesperrt.
    ...
    if ('IntersectionObserver' in window) {
      ...
      // A 30%-threshold can never fire when a tall visual is viewed at 200% zoom:
      // the viewport is physically too short to expose that fraction at once.
      // A small intersection plus modest look-ahead preserves lazy mounting while
      // guaranteeing that every chapter remains reachable at high zoom.
