...
const meta = JSON.parse(await readFile(new URL('../public/data/meta.json', import.meta.url)));
const events = JSON.parse(await readFile(new URL('../public/data/events.json', import.meta.url)));
...
if (!['open', 'permissioned'].includes(meta.publication?.status)) {
  fail(`publication.status=${meta.publication?.status ?? 'missing'}`);
}
if (!meta.publication.publicBuild) fail('build is not marked publicBuild=true');
...
if (!meta.analysis?.outcome?.available || !meta.fits?.perCapita) {
  fail('no complete public outcome model for the impact story');
}
...
// Leck-Guard: EM-DAT-exklusive Felder und -Texte dürfen im offenen Build nie auftauchen
// (Spiegel von EMDAT_FIELDS in scripts/pipeline/outputs.py).
const restrictedFields = new Set([
  'disno', 'magnitude', 'damage_kusd', 'damage_adj_kusd', 'deaths',
]);
...
console.log(`Publication gate passed (${meta.publication.status})`);
